...
def load_data_file(file):
    ...
    try:
        if file_extension == 'csv':
            ...
            if data_dict:
                st.warning("⚠️ Using fallback method - data may not be perfect")
                return data_dict
            
            # If all methods fail, return error indicator
            if 'excel_read_error' in data_dict:
                return data_dict
            else:
                return {'excel_read_error': True}
...
def main():
    ...
    **📊 Excel Support**
    ...
    if uploaded_file is not None:
        ...
        # Check if there was an error reading Excel
            ...
            **Solutions:**
            1. Try saving as a simpler Excel format (.xlsx)
            2. Convert to CSV (recommended)
            3. Check if the file opens in Excel normally
            """)
